...
def test_structural_dedp_valid():
    inv = {"invoice_no": "25612000000068122997", "invoice_date": "2025-12-15",
           "invoice_amount": 469, "invoice_seller": "星巴克"}
...
def test_structural_old_style_year_mismatch():
    ...
    inv = {"invoice_code": "033001900111", "invoice_no": "25894637",
...
def test_qr_csv_format_parse_and_crosscheck():
    qr = IV.parse_invoice_qr_payload("01,10,033001900111,25894637,88.66,20200518,04934")
    assert qr["invoice_code"] == "033001900111"
    assert qr["invoice_no"] == "25894637"
    assert qr["amount"] == 88.66
    assert qr["invoice_date"] == "2020-05-18"
    issues = IV.cross_check_qr({"invoice_no": "25894637", "amount": 88.66}, qr)
    assert issues == []
    bad = IV.cross_check_qr({"invoice_no": "25894637", "amount": 999}, qr)
    assert bad and bad[0]["level"] == "FAIL" and "金额" in bad[0]["rule"]


def test_qr_json_and_dedp_url_formats():
    j = IV.parse_invoice_qr_payload('{"发票号码": "25612000000068122997", "金额": 469}')
    assert j["invoice_no"] == "25612000000068122997" and j["amount"] == 469
    u = IV.parse_invoice_qr_payload("https://inv-veri.chinatax.gov.cn/x?id=25612000000068122997&date=2025-12-15")
    assert u["invoice_no"] == "25612000000068122997"
...
def test_to_text_render():
    r = IV.verify({"invoice_no": "25612000000068122997", "invoice_date": "2025-12-15",
                   "invoice_amount": 469, "invoice_seller": "星巴克"}, settings=None)
    text = IV.to_text(r)
